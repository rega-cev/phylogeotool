...
public class CompactRootedTree extends AttributableImp implements RootedTree {
     ...
     * that (2 0x80000000|0 2 2 0x80000000|1 0x80000000|2 0x80000000|3), where taxa[0] holds a's taxon etc.
     */
    int[] noSons;
    ...
    private Map<String, Object> aMap(int index) {
        if( all == null ) {
            all = new HashMap<Integer, Map<String, Object>>();
        }
    ...
    private class SimpleRootedEdge extends AttributableImp implements Edge {
       ...
       private int index;
        ...
        Map<String, Object> getExistingMap() {
            final int i = (int)(nodes.length + index);
        ...
        Map<String, Object> getMap() {
            return aMap((int)(nodes.length + index));
        }
    ...
    public CompactRootedTree(RootedTree t) {
        ...
        while( level.size() > 0 ) {
            ...
            for( Node n : level ) {
                ...
                nodes[iNode] = new SimpleRootedNode((int)iNode);
                sons[iNode] = ns > 0 ? (int)decendentslStart : 0;
                for(int l = 0; l < ns; ++l) {
                    parent[decendentslStart + l] = (int)iNode;
                }
    ...
    public List<Edge> getEdges(Node node) {
        ...
        for(int n = 0; n < nSons(index); ++n) {
            final int sindex = (int) (sons[index] + n);
    ...
    Map<String, Object> getExistingMap() {
        final int index = (int) nodes.length;
        if( hasAttributeMap(index) ) {
            return aMap(index);
        }
        return null;
