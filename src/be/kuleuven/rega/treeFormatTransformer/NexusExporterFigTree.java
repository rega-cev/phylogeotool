 ...
 */
public class NexusExporterFigTree {
	 ...
	 */
	public NexusExporterFigTree(Writer writer, Map<String, String> taxonNameToColor) {
		this.writer = new PrintWriter(writer);
		this.writer.println("#NEXUS");
		this.taxonNameToColor = taxonNameToColor;
	}
	...
	public void exportSequences(Collection<? extends Sequence> sequences) throws IOException, IllegalArgumentException {
		...
		if (seqType != null) {
			writer.println("\tformat datatype=" + seqType.getNexusDataType() + " missing=" + seqType.getUnknownState().getName() + " gap="
					+ seqType.getGapState().getCode() + ";");
	...
	private void writeTrees(Collection<? extends Tree> trees, boolean checkTaxa) throws IOException {
		...
		for (Tree t : trees) {
			...
			// TREE & UTREE are depreciated in the NEXUS format in favour of a
			// metacomment
	...
	public void exportMatrix(final DistanceMatrix distanceMatrix) {
		...
		// assume distance matrix is symetric, so save upper part. no method to
		// guarantee this yet
	...
	private StringBuilder appendTaxonName(Taxon taxon, StringBuilder builder) {
		...
		builder.append(name);
		builder.append("[&!color=" + this.taxonNameToColor.get(name) + "]");
		return builder;
	}

	/**
	 * Prepare for writing an alignment. If a taxa block exists and is suitable
	 * for alignment, do nothing. If not, write a new taxa block.
	 * 
	 * @param sequences
	 */
	...
	private void appendTree(RootedTree tree, Node node, StringBuilder builder) {
		if (tree.isExternal(node)) {
			...
			// Don't write root length. This is ignored elsewhere and the nexus
			// importer fails
			// whet it is present.
			if (parent != null) {
				if (tree.hasLengths()) {
	...
	private StringBuilder appendAttributes(Attributable item, String excludeKey, StringBuilder builder) {
		...
		for (String key : item.getAttributeNames()) {
			// we should replace the explicit check for name by something more
			// general.
			// Like a reserved character at the start (here &). however we have
			// to worry about backward
	...
	private Map<String, String> taxonNameToColor;
}
