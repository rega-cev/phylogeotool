...
\subsection{Hover over node in clustered phylogeny}
\begin{figure}[H]
\centering
...
When the 'View Tree' button (located on top of the clustered phylogeny panel) is clicked, a popup window is shown (fig. \ref{fig:view_tree}) \footnote{(Update figure with current view)}that visualizes the phylogeny as a radial phylogenetic tree. The colours in the radial tree correspond to the colours of the clusters in the clustered phylogeny panel.
\\
