...
It is however also possible to install your own instance of the PhyloGeoTool; for this, we refer to the installation manual \footnote{(TODO: add link)}. Such an instance can be used to investigate a dataset on a local computer or to host your own public PhyloGeoTool instance.
...
The tool also provides the possibility to insert new virus strains into the existing phylogenetic tree, allowing users to gain insight in the placement (TODO (Ewoutm, when URL is known): location?) of such new strains without the need to reconstruct the phylogeny.
...
If you experience any problems, please contact us \footnote{TODO: link to the contact section}.
...
\begin{itemize}
  ...
  \item The right panel shows a top-level view of the clustering of the tree (initially the root of the tree), as determined by our clustering algorithm. (TODO: GB: need to say where the readers can find our clustering algorithm)
  A button allows you to visualise the phylogenetic tree in its entirety, colored according to the shown clusters.
  \item The bottom left panel shows a histogram or bar chart depending on the the value distribution of the chosen attribute. If the attribute is represented by a continuous dataset, a histogram will be shown while a discrete dataset is displayed by a bar chart plot.
...
When the user hovers the mouse (TODO: mouse pointer?) over a node in the clustered phylogeny, both the attribute chart and the map are updated. 
The attribute chart will be extended (i.e. extra bars will be shown on top of the original chart for each value) with the data that is located in the cluster over which the mouse (TODO: mouse pointer?) hovers. (TODO: GB: extended, maybe overlaid?)
The map, on the other hand, will only show the geographic distribution of the sequences found in the cluster over which the mouse (TODO: mouse pointer?) hovers.
...
The depth of the navigation is also shown when you hover over the '?' which is shown next to 'Level: '
...
(TODO PL, you alternate between American and British English in the document; do you prefer 'color' or 'colour'?, will check with a spell checker and use 1 of the 2 languages)
\break
\break
On the bottom of the popup window, there are two buttons: 'Export' and 'Exit'.
The button with label 'Export' will call a window similar to the one shown in Figure \ref{fig:export_tree}. There are three options available that help to customize the export.
\item Color Tree: enable/disable coloring of tree based on the clusters' color.

\item Show node tips: include/ecxclude leaf names in the phylogenetic tree.
\item Output Format: the output format in which you want your tree to be downloaded (i.d. PDF, NEXUS, SVG, PNG).
...
Once the Pplacer process finished you can review the result by clicking the link that is sent to you by email.
