...
If you don't want to build the JARs/WAR from source, you can skip the following sections and proceed to section \ref{sec:dm}.
...
Download and install the latest Tomcat version from \url{http://tomcat.apache.org} or install it using apt-get as follows:
For example:
\begin{verbatim}
sudo apt-get update
...
\subsubsection*{Ant}
Download and install the latest Ant version from \url{http://ant.apache.org/} as we will use it to build our project.
...
A rooted binary phylogenetic tree in either Nexus or Newick format, which can be time-stamped but this is not mandatory.
...
Remaining columns may contain additional information/annotation for the taxa in the phylogenetic tree, such as geographic location, virus genotype/subtype, patient attributes (age, ethnicity, \ldots), \ldots \\
...
\end{verbatim}
\subsubsection*{Mafft}
Mafft is being used to profile align the newly uploaded sequences to our reference dataset. 
It can easily be installed through the Ubuntu package manager:
\begin{verbatim}
...
A major goal of the PhyloGeoTool is to cluster the tree and to index the taxa annotations, of which the computation takes a long time and its runtime depends on the size of the phylogenetic tree. 
...
<phylogeotool-settings>
  ...
  <!-- alignmentFile
   ...
   This is only needed in case the use of PPlacer 
